...
resultados = df_maiores_que_zero.groupby(['Sigla_UF', 'PEPR_Indústria (R$)', 'Nome_Municipio', 'Ano']).size().reset_index(name='Contagem')

# Ordenar os resultados por sigla e valor em ordem decrescente
resultados_sorted = resultados.sort_values(by=['Sigla_UF', 'PEPR_Indústria (R$)'], ascending=[True, False])
...
for index, row in top_10_por_sigla.iterrows():
    ...
    print(f"  Valor: {row['PEPR_Indústria (R$)']} - Município: {row['Nome_Municipio']} - Ano: {row['Ano']} - Contagem: {row['Contagem']}")